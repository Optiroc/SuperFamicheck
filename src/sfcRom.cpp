...
sfcRom::sfcRom(const string& path) {
  ...
  image = vector<uint8_t>(0);
  {
    ...
    if (file) {
      ...
      if (imageSize < 0x8000 || imageSize > 0xc00000 || imageSize % 0x8000 != 0)
        return;
  ...
  {
    vector<size_t> possibleHeaderLocations = {0xffb0, 0x7fb0, 0x40ffb0};
    vector<pair<int, size_t>> scoredHeaderLocations = {};

    for (size_t loc : possibleHeaderLocations) {
      int score = scoreHeaderLocation(loc);
      if (score > -2)
        scoredHeaderLocations.emplace_back(score, loc);
    }
...
bool sfcRom::isValid() const { return valid; }
...
int sfcRom::scoreHeaderLocation(size_t loc) const {
  ...
  {
    if (header[0x27] >= 0x05 && header[0x27] <= 0x0f)
      score += 2;
    if (header[0x28] >= 0x0a)
      score += 1;
  }
  ...
  // Proper title characters?
  {
    int validChars = 0;
    for (int i = 0; i < 21; ++i) {
      if (!sjisToString(header[0x10 + i]).empty())
        ++validChars;
    }
    if (validChars == 21)
      score += 2;
...
void sfcRom::getHeaderInfo(const vector<uint8_t>& header) {
  ...
  case 0xf3:
    if (chipsetSubtype == 0x10)
      chipSetInfo = "ROM, CX4";
    break;
  case 0xf5:
    if (chipsetSubtype == 0x00)
      chipSetInfo = "ROM, RAM, SPC7110, Battery";
    if (chipsetSubtype == 0x02)
      chipSetInfo = "ROM, RAM, ST-018, Battery";
    hasRam = true;
    break;
  case 0xf6:
    if (chipsetSubtype == 0x01)
      chipSetInfo = "ROM, ST-010/011, Battery";
    break;
  case 0xf9:
    if (chipsetSubtype == 0x00)
      chipSetInfo = "ROM, RAM, SPC7110, RTC, Battery";
    hasRam = true;
    break;

...
int main(int argc, const char* argv[]) {
    ...
    opt.add(
        ...
        0,     // Number of args expected
        0,     // Delimiter if expecting multiple args
        "Silent operation (unless issues found)", "-s", "--semisilent"
    );
    ...
    if (!opt.gotRequired(badOptions)) {
        for (const auto& badOption : badOptions) {
            cerr << "Missing required option: " << badOption << "\n\n";
        }
        std::cout << usage;
        return 1;
    }
    ...
    if (!opt.gotExpected(badOptions)) {
        for (const auto& badOption : badOptions) {
            cerr << "Missing argument for option: " << badOption << "\n\n";
        }
        std::cout << usage;
        return 1;
    }
    ...
    if (!opt.gotValid(badOptions, badArgs)) {
        for (const auto& badOption : badOptions) {
            cerr << "Invalid argument for option: " << badOption << "\n\n";
        }
        std::cout << usage;
        return 1;
    }
